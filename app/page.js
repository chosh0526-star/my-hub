...
export default function Dashboard() {
  ...
  return (
    ...
    {/* 🔥 [챗봇 플로팅 UI] 고도를 적당히 낮춰서 가장 예쁜 간격을 만듭니다! */}
      ...
      {/* 챗봇 창 */}
      {isChatOpen && (
        // 🔥 [궁극의 픽스] 버튼에 맞춰서 창 높이도 11.5rem으로 부드럽게 세팅!
        <div className="fixed bottom-[11.5rem] right-4 md:right-8 w-[calc(100vw-2rem)] md:w-96 h-[30rem] bg-gray-900 border border-gray-700/50 rounded-3xl shadow-2xl z-40 flex flex-col overflow-hidden animate-in slide-in-from-bottom-5 backdrop-blur-xl">
          {/* 헤더 */}
          ...
          >
              ...
              <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[85%] rounded-2xl p-3 text-sm leading-relaxed ${
                  msg.role === 'user' 
                    ...
                    : 'bg-gray-900 border border-gray-700/50 text-gray-200 rounded-bl-sm whitespace-pre-wrap'
                }`}>
                  {msg.content}
                </div>
              </div>
            ...
            {isChatLoading && (
              <div className="flex justify-start">
                <div className="bg-gray-900 border border-gray-700/50 text-gray-400 rounded-2xl rounded-bl-sm p-4 flex gap-1">
