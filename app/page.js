  ...
  return (
    <div className="flex h-screen bg-[#020617] text-gray-100 font-sans selection:bg-blue-500/30 overflow-hidden">
          ...
          {currentMenu === 'home' && (
            <div className="flex justify-start md:justify-center items-center gap-2 w-[calc(100%+2rem)] md:w-full max-w-5xl overflow-x-auto no-scrollbar py-3 px-4 md:px-0 -mx-4 md:mx-0">
              ...
              <div className="flex gap-1 shrink-0 bg-white/5 p-1 rounded-full border border-white/10 ml-2">
                <button onClick={() => setSortOrder(prev => prev === 'desc' ? 'asc' : 'desc')} className={`p-1.5 rounded-full transition-all ${sortOrder === 'desc' ? 'text-blue-400 bg-blue-500/10' : 'text-purple-400 bg-purple-500/10'}`} title="정렬 변경"><ArrowDownUp size={16} className={sortOrder === 'asc' ? 'rotate-180 transition-transform' : 'transition-transform'} /></button>
      ...
      {currentMenu === 'home' && !isSelectMode && (
        <>
          {filter !== '전체' && filter !== '★즐겨찾기' && ( <button onClick={() => setIsSubfolderModalOpen(true)} className="fixed bottom-28 right-10 w-12 h-12 bg-gray-800 border border-gray-700 text-white rounded-full flex items-center justify-center shadow-lg hover:bg-gray-700 active:scale-95 transition-all z-20 group" title="새 폴더 만들기"><FolderPlus size={20} className="text-blue-400 group-hover:text-blue-300 transition-colors" /></button> )}
